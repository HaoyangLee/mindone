...
def create_parser():
    ...
    parser.add_argument("--set", type=str, required=True, choices=["10M_train", "10M_val", "2M_train", "2M_val"], help="set name")
    ...
    if name.endswith(".parquet"):
        ...
        with open(filepath, "r") as f:
            try:
                ...
                raise Exception(f"Invalid json file: {filepath}. Please download the whole {args.set}/part{args.part} again.")
        json_num += 1
    elif name.endswith(".tar"):
        r = os.popen(f"python -m tarfile -l {filepath} | wc -l").read()  # If error message appears, the tar file is invalid. Please download again.
        if int(r) % 3 != 0:
        ...
        raise Exception(f"Unknown file: {filepath}. The valid file formats are parquet, json and tar. Please delete the file or download the whole {args.set}/part{args.part} again.")

assert (
    parquet_num == json_num == tar_num
), f"The data in {data_dir} is incomplete, because the numbers of parquet, json and tar are not identical. Please download the whole {args.set}/part{args.part} again."

success_rate = download_video_num / meta_video_num

print(f"INFO: If no error message appears, the downloaded data is complete and valid. Otherwise please download the whole {args.set}/part{args.part} again.")
