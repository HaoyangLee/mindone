...
# input_dir = "/data1/webvid-10m/dataset/2M_val/00000"
start = time.time()
...
for part_dir in glob.glob(os.path.join(input_dir, "part*")):
    ...
    for tar in tars:
        ...
        for video in tqdm(glob.glob(os.path.join(video_dir, "*.mp4"))):
            ...
            ffmpeg_cmd = f"ffmpeg -threads 8ll -i {video} {frames_dir}/%d.jpg > /dev/null 2>&1"
            os.popen(ffmpeg_cmd)

# for root, dirs, files in os.walk(input_dir):
#     if os.path.basename(root).startswith("part"):
#         tars = glob.glob(os.path.join(root, "*.tar"))
#         for tar in tars:
end = time.time()
